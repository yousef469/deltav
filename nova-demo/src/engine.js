...
export const findNearestCivilization = (lat, lon) => {
...
export const getCelestialGuidance = (currentLat, currentLon, targetLat, targetLon, lang = 'en') => {
    ...
    const messages = {
        en: {
            ...
            left: (d) => `Keep the star ${d}° to your LEFT.`,
            behind: "Walk with the star directly BEHIND you.",
            right: (d) => `Keep the star ${d}° to your RIGHT.`
        },
        ar: {
            straight: "امشِ مباشرة باتجاه النجم.",
            left: (d) => `اجعل النجم ${d} درجة على يسارك.`,
            behind: "امشِ والنجم خلفك مباشرة.",
            right: (d) => `اجعل النجم ${d} درجة على يمينك.`
        },
        zh: {
            straight: "笔直朝星星走去。",
            left: (d) => `保持星星在你的左侧 ${d}°。`,
...
export const validateMove = (piece, from, to, board) => {
